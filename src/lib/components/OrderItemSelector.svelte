<script lang="ts">
	...
	let { items, selectedItems, onUpdate }: {
	...
	// Group items by category (exclude add-ons from main list since they're in the popup)
	...
	function confirmAdd() {
		...
		// Add the main item
		const current = getQuantity(customizeItem.id);
		onUpdate(customizeItem.id, current + 1);

		// Add selected add-ons
		for (const addOnId of selectedAddOns) {
			const addOnCurrent = getQuantity(addOnId);
			onUpdate(addOnId, addOnCurrent + 1);
		}
	...
	function decrement(itemId: string) {
		...
		if (current > 0) {
			onUpdate(itemId, current - 1);
		}
	}

	function directIncrement(itemId: string) {
		const current = getQuantity(itemId);
		onUpdate(itemId, current + 1);
	}
</script>
						...
						<div class="quantity-controls" role="group" aria-label="Quantity for {item.name}">
							...
							{:else}
								<button
									type="button"
									class="qty-button"
									onclick={() => directIncrement(item.id)}
...
{#if showCustomize && customizeItem}
	...
	<div class="customize-popup" role="dialog" aria-modal="true" aria-label="Customize {customizeItem.name}">
		...
		<div class="addons-section">
			<h4 class="addons-title">Add-Ons (optional)</h4>
...
<style>
	...
	.category-header {
		...
		border-bottom: 1px solid rgba(26, 63, 8, 0.2);
	}
